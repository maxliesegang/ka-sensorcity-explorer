...
- **`src/utils/soilFieldBaseline.ts`** — a soil baseline is a column, not a number: every difference is taken at the same depth. The DWD Rheinstetten option is deliberately absent (it measures air); `withSyntheticBaselineOptions` offers only the synthetics a field supplies a label for.
